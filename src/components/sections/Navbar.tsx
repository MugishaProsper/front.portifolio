          ...
          >
            <h3 className="text-2xl font-bold">
              {isDark ? (
                ...
                <Image src="/logo_dark.svg" alt="logo" width={100} height={40} />
              )}
            </h3>
          </motion.div>
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navigation.map((item) => (
                <motion.button
                  ...
                  className={`${isDark ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'} px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200`}
                >
                  {item.name}
                </motion.button>
              ))}
          ...
          <div className="hidden md:flex items-center space-x-4">
            {socialLinks.map((social) => (
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className={`${isDark ? 'text-gray-300' : 'text-gray-700'} ${social.color}`}
              >
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navigation.map((item) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => scrollToSection(item.href)}
                className={`${isDark ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'} block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200 w-full text-left`}
              >
                {item.name}
              </motion.button>
            ))}
            <div className="flex space-x-4 px-3 py-2">
              {socialLinks.map((social) => (
                <motion.a
                  ...
                  rel="noopener noreferrer"
                  whileTap={{ scale: 0.95 }}
                  className={`${isDark ? 'text-gray-300' : 'text-gray-700'} ${social.color} transition-colors duration-200 p-2 rounded-lg hover:bg-white/10`}
                >
                  <social.icon className="w-5 h-5" />
                </motion.a>
              ))}
